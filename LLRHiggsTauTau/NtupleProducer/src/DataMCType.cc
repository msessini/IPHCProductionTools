...
unsigned int DataMCType::GetType(TString name){
  ...
  if(name=="w_lnu" || name=="w_1qlnu" || name=="w_2qlnu" || name=="w_3qlnu" || name=="w_4qlnu")     return W_lnu;
  ...
  if(name=="dy_ll" || name=="dy_1qll" || name=="dy_2qll" || name=="dy_3qll" || name=="dy_4qll" || name=="dy_ll_10to50")     return DY_ll;
...
TString DataMCType::GetName(TString name) {
  name.ToLower();
  StoreType(name);
  return name;
}
